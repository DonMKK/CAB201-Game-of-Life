    ...
    class Program
    {
            ...
            ref int[,] GameArray, ref double RandFactor)
        {
            if (InputFile != "")
            {
                ...
                {
                    Console.WriteLine(Contents[index].Trim().Split(':')[1]);
                    columnsList.Add(int.Parse(Contents[index].Trim().Split(':')[1]));

                    rowsList.Add(int.Parse(Contents[index].Trim().Split(' ')[0]));
        ...
        static void NeighbourCells(ref int AliveOrDead, ref int ChangeLocation, int GameState, bool KnowStep, List<int> Birth, List<int> Survival)
        {
            ...
            //for (int x = 0; x < Survival.Count; x++) 
            //{
            //    Console.WriteLine(Survival[x]);
            //}
        }
        ...
        static void ActualGame(ref int Rows, ref int Columns, ref int[,] GameArray,
            ref bool PeriodicBehaviour, ref int[,] TempArray, ref int NeighbourhoodOrder,
           ref string Neighbourhood, ref List<int> SurvivalLaw, ref List<int> BirthLaw, ref bool IsCenterCounted, ref bool GhostMode)
        {
            ...
            //    GenCount++;
                ...
                for (int CheckRows = 0; CheckRows < GameArray.GetLength(0); CheckRows++)
                {
                    for (int CheckColumns = 0; CheckColumns < GameArray.GetLength(1); CheckColumns++)
                    {
                        ...
                        if (Neighbourhood == "Moore")
                        {
                            ...
                            for (int x = 1; x <= NeighbourhoodOrder; x++)
                            {
                                ...
                                {
                                    GameLogicHelper(ref GameArray, CheckRows - x, CheckColumns + y, Rows, Columns, PeriodicBehaviour, x, ref AliveNumber);
                                ...
                                GameLogicHelper(ref GameArray, CheckRows, CheckColumns - x, Rows, Columns, PeriodicBehaviour, x, ref AliveNumber);

                                GameLogicHelper(ref GameArray, CheckRows, CheckColumns + x, Rows, Columns, PeriodicBehaviour, x, ref AliveNumber);

                                GameLogicHelper(ref GameArray, CheckRows - x, CheckColumns, Rows, Columns, PeriodicBehaviour, x, ref AliveNumber);
                        ...
                        else
                        {
                            ...
                            for (int x = 0; x < NeighbourhoodOrder; x++)
                            {
                                ...
                                GameLogicHelper(ref GameArray, CheckRows, CheckColumns + (x + 1), Rows, Columns, PeriodicBehaviour,  x, ref AliveNumber);

                                GameLogicHelper(ref GameArray, CheckRows + (x + 1), CheckColumns, Rows, Columns, PeriodicBehaviour,  x, ref AliveNumber);
                    ...
                    //Console.WriteLine(AliveNumber);

                        NeighbourCells(ref GameArray[CheckRows, CheckColumns], ref TempArray[CheckRows, CheckColumns], AliveNumber, GhostMode, BirthLaw, SurvivalLaw);
                        //NeighbourCells(ref int AliveOrDead, ref int ChangeLocation, int GameState, bool KnowStep, List<int> Birth, List<int> Survival)
                    }
                }
        ...
        static void Main(string[] args)
        {
            ...
            //if(reader.ReadLine().Contains("Version=#1.0"))
            //{
            //    Seed(ref GameArray,ref CheckAllConditions, reader);
            //}
            //else
            //{
            //    SeedFile02(ref GameArray, ref CheckAllConditions, reader);
           // }
        ...
        ref SurvivalLaw, ref BirthLaw, ref SurvivalLawSpecified, ref BirthLawSpecified, ref GhostMode);
            ...
            ref PeriodicBehaviour, ref TempArray, ref NeighbourhoodOrder,
            ref Neighbourhood, ref SurvivalLaw, ref BirthLaw, ref IsCenterCounted, ref GhostMode);
